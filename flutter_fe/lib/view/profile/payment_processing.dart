...
class _PaymentProcessingPageState extends State<PaymentProcessingPage> {
  ...
  Future<void> _handleDeepLink(Uri? uri) async {
    ...
    if (amount != null && transactionId != null) {
      ...
      if(result == "Your Payment has been Deposited Successfully. You can now create new Tasks."){
        _showStatusModal(context, result, Icon(
          Icons.check_circle,
          color: Colors.green,
          size: 70,
        ));
        ...
        if(role == "Tasker"){
          Navigator.pushAndRemoveUntil(
            context,
            MaterialPageRoute(
                builder: (context) => ServiceAccMain()
            ),
            (route) => false,
          );
        }else if(role == "Client"){
          Navigator.pushAndRemoveUntil(
            context,
            MaterialPageRoute(
                builder: (context) => BusinessAccMain()
            ),
            (route) => false,
          );
        }
  ...
  Future<void> _processPayment(BuildContext parentContext, String paymentMethod) async {
    if (_selectedPaymentMethod.isEmpty || !mounted) {
      _showStatusModal(context, "Please Select a Payment Method", Icon(
        Icons.error,
        color: Colors.red,
      ));
      return;
    ...
    try {
      ...
      if (widget.transferMethod == "deposit") {
        ...
        result = await _escrowController.releaseEscrowPayment(userId, paymentMethod);
        // Handle the response
        final isSuccess = result['success'] ?? false;
        if (isSuccess) {
  ...
  void _showStatusModal(BuildContext parentContext, String message, Widget indicator) {
    ...
    showDialog(context: parentContext, builder: (BuildContext childContext) {
