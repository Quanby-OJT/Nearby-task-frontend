...
class ConversationController{
  ...
  Future<List<Conversation>?> getMessages(BuildContext context, int taskTakenId) async{
    ...
    if(messages.containsKey("messages")){
      ...
      List<Conversation> conversation = message.map((conversation) => Conversation.fromJson(conversation)).toList();
      return conversation;
    }

    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(messages['error'] ?? "Something Went Wrong while Retrieving Your Tasks."))
    );
